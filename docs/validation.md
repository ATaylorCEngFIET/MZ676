...
- Eight Python protocol tests pass. The GUI preview and smoke test pass; the refreshed screenshot uses illustrative data and is not a hardware capture.
...
Both board implementations completed bitstream generation successfully. The artifact audit passes for the AMD block inventory, VHDL wrapper, device part, three ILAs, complete System ILA interface maps, routed timing and bus skew.

| Result | Arty S7-50 | SP701 |
|---|---:|---:|
| Input clock configuration | 12 MHz | 33.333333 MHz |
| Worst setup slack | +7.351 ns | +9.009 ns |
| Worst hold slack | +0.019 ns | +0.032 ns |
| Worst pulse-width slack | +8.750 ns | +8.870 ns |
| Unconstrained internal endpoints | 0 | 0 |
| Slice LUTs | 10,992 / 32,600 (33.72%) | 11,003 / 64,000 (17.19%) |
...
Current artifacts are `build/ipi_arty_s7_50/debug_lab.bit` + `.ltx` and `build/ipi_sp701/debug_lab.bit` + `.ltx`. Reports and the generated `.xpr` are beside them. SHA-256 hashes are in `build/artifact_sha256.json`.
